...
public class App {
    ...
    private static void printMenu() {
        System.out.println();
        System.out.println("-------------------------------------------");
        System.out.println("Logged in as: " + (loggedInCustomer == null ? "guest (not logged in)" : loggedInCustomer.getCustomerName()));
    ...
    /**
     * Simple login-by-email since the customers table has no password column.
     * If the email isn't found, the customer is registered on the spot.
     */
    private static void login() throws SQLException {
        ...
        int newId = customerDAO.registerCustomer(newCustomer);
        newCustomer.setCustomerId(newId);
        loggedInCustomer = newCustomer;
        System.out.println("Registered and logged in as " + name + " (customer_id=" + newId + ")");
    }
    ...
    private static void removeFromCart() throws SQLException {
    ...
    private static void placeOrder() throws SQLException, ClassNotFoundException {
        int orderId = orderDAO.placeOrder(loggedInCustomer.getCustomerId());
        System.out.println("Order placed successfully! Order ID: " + orderId);
    }

    private static void viewOrderHistory() throws SQLException {
        List<Order> orders = orderDAO.viewOrders(loggedInCustomer.getCustomerId());
        if (orders.isEmpty()) {
    ...
    /** Thrown internally when an action requiring login is attempted while logged out. */
